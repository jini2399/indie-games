...
function PixelCapybara({ size = 80, isAttacking = false, animState = { state: "idle", frameIndex: 0 } }: { size?: number; isAttacking?: boolean; animState?: CapybaraAnimState }) {
  ...
  useEffect(() => {
    ...
    const img = new Image();
    // basePath 포함된 경로
    img.src = "/indie-games/capybara-sprites.png";
    ...
    img.onload = () => {
      ...
      // 60x60 프레임을 size 크기로 그리기
      ctx.imageSmoothingEnabled = false;
      ctx.drawImage(img, x, y, 60, 60, 0, 0, size, size);
    };

    img.onerror = () => {
      console.error("Failed to load capybara sprite image");
    };
  }, [size, animState]);

  return <canvas ref={canvasRef} style={{ imageRendering: "pixelated", backgroundColor: "transparent" }} />;
}
